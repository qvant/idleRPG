from .messages import M_RESURRECTED, M_KILLED_BY_ENEMY, M_DIED, M_DRINK_HEALTH_POTION, M_DRINK_MANA_POTION, \
    M_RUN_AWAY, M_RUN_AWAY_FAILED, M_CASTED_SPELL, M_CASTED_SPELL_ON_HIMSELF, M_FOUND_LOOT, M_GP, M_POINT, \
    M_BOUGHT_HEALTH_POTIONS, M_REACHED_LEVEL, M_BOUGHT_MANA_POTIONS, M_BOUGHT_EQUIPMENT, M_RESTED, M_USED_ABILITY, \
    M_USED_ABILITY_STATUS, M_COMPLETED_QUEST, M_ACCEPTED_QUEST, M_POTION
...
class Event:
    ...
    def __str__(self):
        if self.type == EVENT_TYPE_RESURRECTED:
            ...
            return self.player.trans.get_message(M_CASTED_SPELL,
                                                 self.player.locale).format(self.player.name,
                                                                            self.player.trans.get_message(
                                                                                self.spell.name, self.player.locale),
                                                                            self.player.trans.get_message(
                                                                                self.enemy.name, self.player.locale,
                                                                                is_genitive=True), self.damage)
            ...
            return self.player.trans.get_message(M_BOUGHT_HEALTH_POTIONS,
                                                 self.player.locale).format(self.player.name,
                                                                            self.potion_number,
                                                                            self.player.trans.get_message(
                                                                                M_POTION,
                                                                                self.player.locale,
                                                                                connected_number=self.potion_number))
        elif self.type == EVENT_TYPE_BOUGHT_MANA_POTIONS:
            return self.player.trans.get_message(M_BOUGHT_MANA_POTIONS,
                                                 self.player.locale).format(self.player.name,
                                                                            self.potion_number,
                                                                            self.player.trans.get_message(
                                                                                M_POTION,
